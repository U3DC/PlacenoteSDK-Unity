...
public class FeaturesVisualizer : MonoBehaviour, PlacenoteListener
{
    ...
    [SerializeField] private Material mPtCloudMat;
    [SerializeField] private GameObject mMap;
    [SerializeField] private GameObject mPointCloud;
    [SerializeField] private bool mEnableMapPoints = false;
    [SerializeField] private bool mEnableMesh = true;
    [SerializeField] private float mMeshVisualizationRadius = 5f;
    [SerializeField] private int FixVertexCount = 2000;

    public Material m_ClearMaterial;
    public UnityARVideo arVideo;
    private Matrix4x4 _displayTransform;

    public static UnityAction<GameObject> OnMeshBlockAdded;
    ...
    public void OnPose( Matrix4x4 outputPose, Matrix4x4 arkitPose )
    {
        ...
        {
            Mesh mesh = mMeshBlocks[key].GetComponent<MeshFilter>().sharedMesh;
    ...
    public void OnDenseMeshBlocks( Dictionary<LibPlacenote.PNMeshBlockIndex, LibPlacenote.PNMeshBlock> meshBlocks )
    {
        ...
        foreach( KeyValuePair<LibPlacenote.PNMeshBlockIndex, LibPlacenote.PNMeshBlock> entry in meshBlocks )
        {
            ...
            else
            {
                ...
                meshObj.GetComponent<MeshRenderer>().material = m_ClearMaterial;
                mMeshBlocks.Add( entry.Key, meshObj );
                mMeshBlockStatus.Add( entry.Key, true );
                OnMeshBlockAdded.Invoke( meshObj );
            }
    ...
    public void OnDensePointcloud( LibPlacenote.PNFeaturePointUnity[] densePoints )
    {
        ...
        mf.sharedMesh.SetIndices( indices, MeshTopology.Points, 0 );
